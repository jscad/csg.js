...
test.only('vec2: divide() called with two paramerters should return a vec2 with correct values', t => {
  ...
  const exp3 = fromValues(6, 3)
  t.deepEqual(obs3, exp3)

  const obs4 = divide([-6, -6], [1, 2])
  const exp4 = fromValues(-6, -3)
  t.deepEqual(obs4, exp4)

  const obs5 = divide([6, 6], [-1, -2])
  const exp5 = fromValues(-6, -3)
  t.deepEqual(obs5, exp5)

  const obs6 = divide([-6, -6], [-1, -2])
  const exp6 = fromValues(6, 3)
  t.deepEqual(obs6, exp6)
})

test.only('vec2: divide() called with three paramerters should update a vec2 with correct values', t => {
