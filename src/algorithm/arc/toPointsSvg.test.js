...
const toPointsSvg = require('./toPointsSvg')
const vec2 = require('../../math/vec2')

// TODO: More comprehensive test cases.

test('Flat arc', t => {
  t.deepEqual(toPointsSvg({}, [0, 0], [1, 1]).map(p => [...p]),
              [[0, 0], [1, 1]])
})

test('Half circle, small arc', t => {
  t.deepEqual(toPointsSvg({ resolution: 6, large: false, xRadius: 0.5, yRadius: 0.5 }, [1, 1], [2, 2]).map(p => [...p]),
              [[1, 1],
...
test('Half circle, large arc', t => {
  t.deepEqual(toPointsSvg({ resolution: 6, large: true, xRadius: 0.5, yRadius: 0.5 }, [1, 1], [2, 2]).map(p => [...p]),
              [[1, 1],
...
test('Quarter circle', t => {
  t.deepEqual(toPointsSvg({ resolution: 6, large: false, xRadius: 1, yRadius: 1 }, [1, 1], [2, 2]).map(p => [...p]),
              [[1, 1],
...
test('Three-quarter circle', t => {
  t.deepEqual(toPointsSvg({ resolution: 6, large: true, xRadius: 1, yRadius: 1 }, [1, 1], [2, 2]).map(p => [...p]),
              [[1, 1],
